var actorChars = {
  '@': Player,
  'o': Coin,
  'p': CoinTwo //Declaring a second coin that reveals when the 'blind' square is stepped on
};
...
function Level(plan) {
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Adding in the extra fieldtypes for the game
      else if (ch === "i") {
        fieldType = "ice";
      }
      else if (ch === "y") {
...
//setting both coins movement/animation. They will act identical
function Coin(pos) {
  this.basePosition = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  this.wobble = Math.random() * Math.PI * 2;
}
Coin.prototype.type = 'coin';

function CoinTwo(pos) {
  this.basePosition = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  this.wobble = Math.random() * Math.PI * 2;
}
CoinTwo.prototype.type = 'coinTwo';
...
var wobbleSpeed = 8, wobbleDist = 0.02;
...
CoinTwo.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.pos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveY = function(step, level, keys, pos) {
  ...
  if (obstacle === "lava") {
    this.pos = new Vector(7.2, 18.5);
  }
  if (obstacle === "bouncer") {
    this.speed.y = -jumpSpeed - 5;
  ...
  if (obstacle === "blind") {
      if (changeBackground[0].style.background != 'rgb(203, 232, 107)') {
        changeBackground[0].style.background = 'rgb(203, 232, 107)';
        console.log('blind activated');
      }
  }
  else if (lastObstacle === "blind"){
      changeBackground[0].style.background = 'white';
      console.log('blind deactivated');
  }
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "coinTwo") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {return actor.type == "coin";}) && !this.actors.some(function(actor) {return actor.type == "coinTwo";})) {
      this.status = "won";
      this.finishDelay = 1;
    }
  }
...
var arrowCodes = {37: "left", 65: "left", 97: "left", 38: "up", 87: "up", 119: "up", 39: "right", 68: "right", 100: "right", 40: "down", 83: "down", 115: "down"};
